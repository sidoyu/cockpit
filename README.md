...
> **현 빌드의 기능 기본값(v0.1.1, WSL2 골든 이미지)**: bypass=**ON**(사전적용) · effort=xhigh · model=`claude-opus-4-8[1m]` · 원격조종(claude.ai Remote Control)=**ON**(아웃바운드 HTTPS 전용·수신포트 없음) · trust=자동 수락. 단 **외부 송신(egress)=OFF**(첫 실행 "동의 한 화면") · 보조 검토(Codex)=비활성(스위치식) · **자체호스팅 대시보드=비활성(명시 opt-in)**. 끄기/상태: GOVERNANCE.md 4·6장.
>
> **⚠ 출고 상태(v0.1.1 변경)**: 편의 설정(bypass·effort·model·원격조종·trust)은 동료가 "로그인만" 하도록 이미지에 **사전적용 출고**한다(2026-06-26 결정). **끝까지 OFF 유지 불변식**(이미지에 절대 굽지 않음)은 ① egress 동의 마커 ② Codex 스위치 ③ 자체호스팅 대시보드 자동시작 ④ 비밀/자격증명 — `scripts/smoke-image.sh` 가 빌드마다 검증한다. (bypass 는 CLAUDE.md "멈춰 질문" 1차 방어와 **반드시 동반**해 출고된다.)
...
> **상태**: v0.1.1 — 미서명+체크섬 발행(코드서명 인증서 미보유, 무결성은 SHA-256 핀 대조). 공개 repo `github.com/sidoyu/cockpit`.
