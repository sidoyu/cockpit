...
- **환자정보·PII·기밀 입력 금지**, 사용 위치·용도는 본인 판단·책임
- bypass·원격·외부 송출을 이해
...
### 1.6 기존 Claude Code 기억 이관 (이미 쓰던 사람이라면 — 선택)

이미 다른 곳에서 Claude Code 를 쓰며 `CLAUDE.md`·기억을 쌓아온 사용자라면 그것을 가져올 수 있다.

> **이건 로컬 파일 복사다. API 키도, 외부 송신도 필요 없다.**
> (3.5 의 '새 기억 자동추출'은 별개 기능이다 — 그쪽만 키와 외부 송신 동의가 필요하다.
>  "키를 넣어야 기억이 온다"는 오해를 만들지 말 것.)

```
python3 "${CLAUDE_PLUGIN_ROOT}/skills/setup-wizard/import_existing.py" detect   # 소스 탐색(읽기 전용)
python3 ".../import_existing.py" plan                                           # 기본 dry-run: 무엇이 복사/병합/격리되는지
python3 ".../import_existing.py" apply --claude-md merge                        # 사용자 확인 후 실행
```

- `detect` 가 Windows 프로필(`%USERPROFILE%\.claude`)을 자동으로 찾는다. 못 찾으면 `--source <경로>`.
  다른 WSL 배포판은 **자동 대상이 아니다**(안내만) — 그 안에서 `/mnt/c` 로 꺼낸 뒤 `--source` 로 지정.
- `plan` 은 `발견 / 적용예정 / 건너뜀 / 충돌·격리 / 사용자 선택 필요 / 위험경고 / 원복위치` 를 보여준다.
  **반드시 사용자에게 보여주고 확인을 받은 뒤** `apply`.
- `--claude-md` 3지선다: `merge`(기본, cockpit 규칙 + 이전 개인 규칙을 구분 섹션으로 흡수) ·
  `cockpit`(cockpit 규칙만) · `mine`(이전 규칙으로 교체 — **cockpit 자동동작·규율 대부분이 설명되지 않은 채 남는다고 경고할 것**).
- 진짜 모순(예: 이전 규칙 "영어로" ↔ cockpit "한국어")은 자동 해소하지 않는다. plan 이 해당 줄을 짚어주면 **사용자에게 판단을 넘긴다**.
- 안전: 소스는 읽기 전용 · 삭제 0 · 덮어쓰기 0 · apply 직전 `backup.py` 자동 백업 · staging 에서 색인 재생성이
  통과해야만 실물 배치 · 재실행 멱등. 자격증명(`.credentials.json`)·앱 상태(`~/.claude.json`)·트랜스크립트는 복사하지 않는다.
- 훅·MCP 서버는 **자동 병합하지 않는다**(원본이 Windows 명령이면 WSL 에서 매 세션 실패). plan 이 목록만 보고한다.

**이 배포판에서 이미 쓰던 기억이 흩어져 있다면**(배선 수정 전 버전을 쓰던 경우):
```
python3 ".../import_existing.py" adopt-native            # dry-run
python3 ".../import_existing.py" adopt-native --apply
```

### 1.7 기억 배선 확인 (doctor 가 "기억 배선 끊김"을 표시했다면)

```
python3 ".../setup.py" wire-auto-memory            # dry-run
python3 ".../setup.py" wire-auto-memory --apply
```
배선이 끊겨 있으면 `MEMORY.md` 색인이 **어떤 세션에도 로드되지 않는다**(기억을 쌓아도 Claude 가 못 봄).
신규 이미지는 provision 이 미리 굽는다. 이 명령은 이전 버전에서 올라온 환경을 고친다. 적용은 **새 세션부터** 반영된다.
