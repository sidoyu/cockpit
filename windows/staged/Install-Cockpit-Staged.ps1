...
.DESCRIPTION
  ...
  골든 경로와 동일 안전 원칙:
    ...
    • 위험 기능 OFF 출고(provision.sh 강제). 켜기는 첫 실행 /cockpit-setup 동의 게이트.
...
Info "스테이지드 설치 완료 ✓  (위험 기능 OFF)"
Write-Host ""
Write-Host "다음 단계(배포판 안에서):" -ForegroundColor Green
Write-Host "  wsl -d $DistroName"
Write-Host "  claude   →  /plugin marketplace add <소스>  →  /plugin install cockpit@cc-companion  →  /cockpit-setup"
Write-Host ""
